...
pub struct SetChromaStatic {
    pub r: u8,
    pub g: u8,
    pub b: u8,
}
pub struct SetChromaSpectrum;
pub struct SetChromaWave {
    pub direction: i32,
}
pub struct SetChromaBreath {
    pub r: u8,
    pub g: u8,
    pub b: u8,
}
...
impl Capability for GetBrightness {
    fn execute<'a>(
        ...
        Box::pin(async move {
            Ok(CapabilityResponse::Float(
                backend.get_brightness(serial).await?,
            ))
        })
    }
}
...
impl Capability for SetChromaStatic {
    fn execute<'a>(
        ...
        Box::pin(async move {
            backend
                .set_chroma_static(serial, self.r, self.g, self.b)
                .await?;
            Ok(CapabilityResponse::Ok)
        })
    }
...
impl Capability for SetChromaBreath {
    fn execute<'a>(
        ...
        Box::pin(async move {
            backend
                .set_chroma_breath(serial, self.r, self.g, self.b)
                .await?;
            Ok(CapabilityResponse::Ok)
        })
    }
