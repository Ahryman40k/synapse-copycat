...
fn devices() -> Vec<RazerDevice> {
   /*  let sub = Observer::new()
    ...
    devices.dedup_by( |a, b| (a.product_id == b.product_id && a.vendor_id != b.vendor_id) || (a.product_id != b.product_id && a.vendor_id == b.vendor_id) );

    let mut result = vec![];
    for device in devices  {
        result.push( RazerDevice { vendor_id: device.vendor_id, product_id: device.product_id } );
    }
