...
#[derive(Deserialize)] struct StringResponse  { value: String }
#[derive(Deserialize)] struct IntResponse     { value: i32 }
#[derive(Deserialize)] struct FloatResponse   { value: f64 }
#[derive(Deserialize)] struct BoolResponse    { value: bool }
#[derive(Deserialize)] struct PairResponse    { x: i32, y: i32 }
#[derive(Deserialize)] struct VidPidResponse  { vid: i32, pid: i32 }
#[derive(Deserialize)] struct DeviceList      { serials: Vec<String> }
...
impl RestBackend {
    ...
    async fn get<T: for<'de> Deserialize<'de>>(&self, url: String) -> Result<T, BackendError> {
        ...
        resp.json::<T>().await.map_err(|e| BackendError::Transport(e.to_string()))
    }
...
impl DeviceBackend for RestBackend {
    ...
    fn set_dpi(&self, serial: &str, x: i32, y: i32) -> BoxFuture<'_, Result<(), BackendError>> {
        let url = self.device_url(serial, "dpi");
        Box::pin(async move {
            self.post(url, &serde_json::json!({ "x": x, "y": y })).await
        })
    }
    ...
    fn set_brightness(&self, serial: &str, value: f64) -> BoxFuture<'_, Result<(), BackendError>> {
        let url = self.device_url(serial, "lighting/brightness");
        Box::pin(async move {
            self.post(url, &serde_json::json!({ "value": value })).await
        })
    }

    fn set_chroma_static(&self, serial: &str, r: u8, g: u8, b: u8) -> BoxFuture<'_, Result<(), BackendError>> {
        let url = self.device_url(serial, "lighting/chroma/static");
        Box::pin(async move {
            self.post(url, &serde_json::json!({ "r": r, "g": g, "b": b })).await
        })
    }
    ...
    fn set_chroma_wave(&self, serial: &str, direction: i32) -> BoxFuture<'_, Result<(), BackendError>> {
        let url = self.device_url(serial, "lighting/chroma/wave");
        Box::pin(async move {
            self.post(url, &serde_json::json!({ "direction": direction })).await
        })
    }

    fn set_chroma_breath(&self, serial: &str, r: u8, g: u8, b: u8) -> BoxFuture<'_, Result<(), BackendError>> {
